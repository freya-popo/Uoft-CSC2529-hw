...
def deconv_admm_tv(b, c, lam, rho, num_iters, anisotropic_tv=False):
    ...
    grad_fn = lambda x: np.stack((np.real(ifft2(fft2(x) * dxFT)),np.real(ifft2(fft2(x) * dyFT))), axis=0)

    # precompute the denominator for the x-update
    denom = cTFT * cFT + rho * (dxTFT * dxFT + dyTFT * dyFT)   # you need to edit this, it's just a placeholder
    ...
    for it in tqdm(range(num_iters)):
        ...
        nominator = cTFT * bFT + rho * np.sum(dxyTFT * fft2(z - u), axis=0)
        x = np.real(ifft2(nominator / denom))
